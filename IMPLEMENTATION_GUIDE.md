...
Stage 8 now supplies `ask_policy`, `start_policy_assistant` and `reset_policy_assistant` in the notebook. The source loader has a separate definition/execution step so setup failures can return a checked result. See [Stage 8 handoff](STAGE_8_HANDOFF.md) for statuses, recovery and the distinction between injected failures and actual live observations. Gradio remains Stage 9 work.
