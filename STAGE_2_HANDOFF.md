...
Next session: 30–45 minutes, Maharashtra only. Stage S's preparation batch is finished; source acceptance and current-benefit verification remain open. The user requested progress toward Stage 2. Candidate data may now be used for local extraction tests; this adjustment does not approve it for answers or remove the final verification requirements.
...
4. The team reviews the source packet, records actual corrections/reviewer/date, and writes two source-backed examples independently of model output. Keep them distinct from technical extraction checks. Do not generate expected answers using the assistant.
...
Stage 2 acceptance additionally needs the team's checked source text and two human-verified examples. The current-benefit requirement remains unresolved separately; candidate tests cannot satisfy it. No embeddings, Chroma index, answer generation or UI belongs in this session. Save the observed checks and one focused Git commit, then stop at the Stage 2 boundary.
